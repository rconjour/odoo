...
class document_file(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if vals.get('datas', False):
            vals['file_type'], vals['index_content'] = self._index(cr, uid, vals['datas'].decode('base64'), vals.get('datas_fname', False), vals.get('file_type', None), context)
    ...
    def _index(self, cr, uid, data, datas_fname, file_type, context=None):
        if context is None:
            context = {}
        if not context.get('index_content', True):
            return False, False
