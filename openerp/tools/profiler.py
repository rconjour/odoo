...
class _LogTracer(object):
    ...
    def tracer(self, frame, event, arg):
        ...
        if frame.f_code.co_name in ['<genexpr>', '__getattr__', '__iter__',
                                    '__init__']:
            return
        ...
        if isinstance(in_self, (
                openerp.sql_db.Cursor,
                openerp.sql_db.TestCursor,
                openerp.sql_db.LazyCursor)):
            return self.tracer
        ...
        codeProfile['calls'].append({
            ...
            'queries': cr and cr.sql_log_count or 0,
            'time': time.time(),
...
def profile(method=None, whitelist=None, blacklist=(None,), files=None,
    ...
    """
        ...
        .. code-block:: python
          ...
          from openerp.tools.profiler import profile
          ...
          class SaleOrder(models.Model):
            ...
            ...
            @profile(
                files=['/home/openerp/odoo/odoo/addons/sale/models/sale.py'])
            def write(self):
            ...
    ...
    def _odooProfile(method, *args, **kwargs):
        log_tracer = _LogTracer(whitelist=whitelist, blacklist=blacklist,
                                files=files, deep=deep)
        ...
        for v in log_tracer.profiles.values():
            ...
            for k, call in enumerate(v['calls']):
                ...
                n = k + 1
                while k + 1 <= l and v['calls'][k + 1]['callno'] != call[
                    'callno']:
                    n += 1
                if n >= l:
                    continue
                ...
                if next_call['queries'] is not None:
                    v['report'][call['lineno']]['nb_queries'] \
                        += next_call['queries'] - call.get('queries', 0)
                v['report'][call['lineno']]['delay'] += next_call['time'] - \
                                                        call['time']
            ...
            log += u"\033[1;33m%s %s--------------------- %s, %s\033[1;0m\n\n"\
                   % (v['model'] or '', '-' * (15 - len(v['model'] or '')),
                      v['filename'], v['firstline'])
